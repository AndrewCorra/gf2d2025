...
void gf2d_space_free(Space* space)
{
    ...
    gfc_list_foreach_context(space->staticShapes, gf2d_free_shapes, NULL);
    gfc_list_foreach_context(space->dynamicBodyList, gf2d_free_dynamic_bodies, NULL);
...
void gf2d_space_draw(Space* space, Vector2D offset)
{
    ...
    gf2d_draw_rect(r, gfc_color(255, 0, 0, 255));
    count = gfc_list_get_count(space->dynamicBodyList);
    for (i = 0; i < count; i++)
    {
        db = (DynamicBody*)gfc_list_get_nth(space->dynamicBodyList, i);
